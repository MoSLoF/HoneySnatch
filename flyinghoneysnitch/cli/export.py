"""Export commands for FlyingHoneySnitch CLI.

Provides `fhs export` subcommands for exporting scan data
...
def export_csv(db_path: str, output: str | None, data_type: str, encrypt: bool, passphrase: str | None) -> None:
    ...
    if output is None:
        output = f"fhs_export_{datetime.now():%Y%m%d_%H%M%S}.csv"
...
def export_json(db_path: str, output: str | None, encrypt: bool, passphrase: str | None) -> None:
    ...
    if output is None:
        output = f"fhs_export_{datetime.now():%Y%m%d_%H%M%S}.json"
...
def export_kml(db_path: str, output: str | None, encrypt: bool, passphrase: str | None) -> None:
    ...
    if output is None:
        output = f"fhs_export_{datetime.now():%Y%m%d_%H%M%S}.kml"
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>FlyingHoneySnitch Scan - {_xml_escape(session.name)}</name>
...
def _encrypt_output(output_path: str, passphrase: str | None) -> None:
    ...
    from flyinghoneysnitch.utils.crypto import encrypt_file
