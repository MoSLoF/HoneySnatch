"""Scan commands for the FlyingHoneySnitch CLI.

Provides `fhs scan` subcommands for starting/stopping wireless scans,
...
from flyinghoneysnitch.core.models import ScanEvent
...
def start_scan(
    ...
    Requires a wireless interface in monitor mode. Use `fhs scan list-interfaces`
    to see available interfaces.

    Examples:

        fhs scan start -i wlan0mon

        fhs scan start -i wlan0mon -c 1,6,11 -d 60

        fhs scan start -i wlan0mon --capture /tmp/captures
    """
    ...
    # Initialize scanner
    from flyinghoneysnitch.core.scanner import WifiScanner
    ...
    from flyinghoneysnitch.db.database import create_session_db
...
def enable_monitor(interface: str) -> None:
    ...
    from flyinghoneysnitch.utils.interfaces import enable_monitor_mode
    from flyinghoneysnitch.utils.permissions import check_permissions
...
def disable_monitor(interface: str) -> None:
    ...
    from flyinghoneysnitch.utils.interfaces import disable_monitor_mode
