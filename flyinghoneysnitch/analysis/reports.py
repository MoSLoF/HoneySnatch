...
from flyinghoneysnitch.analysis.patterns import PatternAnalyzer
from flyinghoneysnitch.analysis.profiles import ProfileEngine
from flyinghoneysnitch.core.models import ScanSession
...
<head>
    <meta charset="UTF-8">
    <title>FlyingHoneySnitch Report - {session.name}</title>
    <style>
...
<body>
    <h1>FlyingHoneySnitch Scan Report</h1>
    ...
    <footer style="margin-top: 40px; color: #666; border-top: 1px solid #333; padding-top: 10px;">
        Generated by FlyingHoneySnitch v0.1.0 at {datetime.now().isoformat()}
    </footer>
</body>
</html>"""
...
def generate_summary_text(session: ScanSession) -> str:
    ...
    lines = [
        f"FlyingHoneySnitch Scan Summary",
        f"{'=' * 40}",
